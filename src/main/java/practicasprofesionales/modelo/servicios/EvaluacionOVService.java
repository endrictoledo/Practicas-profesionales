...
public class EvaluacionOVService {
    ...
    public RespuestaOperacion procesarEvaluacion(int idUsuario,
            String satisfaccion, String involucramiento, String objetivos,
                                                            String sugerencia) {
        ...
        if (satisfaccion == null || satisfaccion.trim().isEmpty() ||
            ...
            return new RespuestaOperacion(true, "Por favor,"
                    + " responda todas las preguntas para continuar.");
        }

        try {
            int idExpediente = documentoDAO.obtenerIdExpedientePorUsuario(
                                                                    idUsuario);
            if (idExpediente == -1) {
                return new RespuestaOperacion(true, "El estudiante no tiene "
                        + "un expediente asignado."
                        + " No se puede guardar la evaluación.");
            }

            
            String contenidoArchivo = 
                  "=== EVALUACIÓN DE ORGANIZACIÓN VINCULADA ===\n\n" +
            ...
            int idCatalogo = documentoDAO.obtenerIdCatalogoDocumento(
                    "Evaluación OV");
            if (idCatalogo == -1) {
                return new RespuestaOperacion(true, "No se encontró el tipo "
                        + "de documento 'Evaluación OV' en la base de datos.");
            }
            ...
            if (exito) {
                ...
                return new RespuestaOperacion(false, "Has realizado la "
                        + "evaluación correctamente.", archivoBytes);
            } else {
                return new RespuestaOperacion(true, "No se pudo registrar "
                        + "la evaluación en la base de datos.");
            }

        } catch (ExcepcionDAO e) {
            e.printStackTrace();
            return new RespuestaOperacion(true,
                    "Error interno de base de datos: " + e.getMessage());
        }
    }
}
